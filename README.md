...
**逐章详纲**：写第 N 章前先把该章细化成 500~700 字的结构化详纲（`[任务]` / `[场景N]` / `[边界]` / `[钩子]` / `[字数]`），落库后弹在写作区顶部等作者确认（可手改 / 重生成 / 采用），采用后才进场景清单与扩写。详纲是本章唯一写作依据，也是把「约 100 字细纲撑起 2000 字正文」（扩写比 1:8~1:10，节奏提前的根因）压到约 1:1.5 的关键。

**逐场景扩写**：详纲已切好场景时由 `parseDetailScenes` 确定性解析出场景清单（零 AI 请求、与详纲 100% 对齐）；解析不出才降级请 AI 规划 3~5 个场景（顺带圈定出场人物、涉及地点、重大分支提案）。确认后每个场景单独成文（600~900 字），根治单次生成长文后段压缩的问题。自动连写模式可整本推进，遇到剧情重大分支会暂停等待作者拍板。
...
| 题材世界观库 | 20 个题材 × 8 字段（世界架构 / 力量体系 / 势力格局 / 地理分层 / 禁忌代价 / 高频套路 / 反套路切口 / 意象词库）约 1000 字，注入分 brief（灵感选题）与 full（圣经生成）两档；套路与反套路清单直喂选题规则，可按题材覆盖并一键恢复内置模板 |
| 逐章详纲与细纲驱动 | 写章前把本章细化为 500~700 字结构化详纲并落库确认；开启「细纲驱动」后单章只依赖详纲 + 世界规则块 + 出场人物卡 + 文风 + 前文尾部 800 字，user 段上下文砍掉约 89%、详纲占比从 3.2% 升到 30.3% |
| 细纲越界监管 | 纯前端零 AI 的 `outlineLeakScan`：封存真相连续重合 ≥15 字判 **blocked** 拒绝入库；未登场势力 / 未解锁地图层 / 抢收本卷禁回收伏笔 / 越界消耗后续章任务 / 提前引爆跨卷里程碑判 **warning** 只提示不阻断。挂在向导第 1 卷细纲、续写细纲、逐章详纲三处 |
...
```text
project
├── 设定：梗概 / 世界观（结构化块）/ 人物档案 / 细纲地图（含起承转合标签）/ 逐章详纲（outlineDetail）/ 细纲驱动开关（outlineDriven）/ 卷档案 / 小说圣经（真相层+线索层+人物锚点）/ 全书章名骨架
...
开启「细纲驱动」后走精简分支：剧情依据换成本章 500~700 字详纲，全书梗概 / 滚动摘要 / 故事线 / 编年史 / 前文召回 / 地图分层全部不注入，详见[细纲驱动模式](#细纲驱动模式)。
...
| 5 章名与细纲 | 全书每章章名+唯一任务（写章时硬约束）+ 第 1 卷**精简细纲地图**（每章 120~200 字，管全书走向与边界感知）。500~700 字的结构化详纲不在这里一次跑完，而是写每章前逐章细化（一次输出上万字会撞模型上限） | 每章只完成一个任务 |
| 6 对账成书 | 零 AI 调用的结构化对账（伏笔闭环表、缺线索提醒），确认后一键成书 | 检查的和写的是同一份事实 |

关键机制：**真相隔离**——圣经真相层（truth）永不进写作上下文，只有线索（clues）可露出，真相仅供审核对照（揭示红线 = 终卷起始章）；**传闻级隔离**——地图分层同一套机制：已解锁层可展开、下一层只能以传闻提及、更高层与真相栏永不进上下文，审核对照防地图越界（根治世界观困死一城）；**回收章锚定**——回收卷按卷内 70% 位置换算为具体章号硬边界，提示词禁止 + 归档拦截双保险；**导入分章**——既有章节按「第 N 章」章头切分、章号重排连续、逐章轻量归档（摘要+伏笔检测），支持从导入内容反推圣经；**细纲输入裁剪**——生成第 1 卷细纲只喂本卷骨架（原先直传全书骨架全文，模型能看到后续所有卷的章名与任务）、续写细纲只喂本卷相关的主线切片（≤800 字）而非全书梗概、逐章详纲只喂本章任务 + 后 2 章任务标题 + 本卷已登场世界盘，`bible.truths` 全程不进任何细纲生成请求；**细纲越界监管**——三处细纲产出统一过 `outlineLeakScan`，红线拒绝入库、其余只提示不阻断（改造前细纲侧只有格式类校验，零剧情越界检查）。旧书不受影响，新机制只对新书生效（无地图分层/骨架时自动降级不注入）。

---

## 细纲驱动模式

长篇写作里最难压住的毛病是**节奏提前**：该在第 30 章揭的事，第 12 章就顺手写掉了。原因不在模型不听话，而在它没东西可写——2000 字的章被切成 2~3 个场景、每场景要产出 850~1000 字，可依的只有整章约 100 字的精简细纲，**扩写比 1:8~1:10**。防提前的负向约束（止步规则、揭示层级、末场景强制收束、草稿自检）已经堆得很密，缺的是正向内容量：模型没料可写就只能推进新剧情。

墨语的解法是把细纲升级为**逐章详纲**，再配一个可选的上下文精简开关。

### 逐章详纲

写第 N 章前若 `outlineDetail[N]` 不存在，自动生成并落库，在写作区顶部弹出可折叠预览（手改 / 重生成 / 采用），确认后才走场景清单 → 扩写：

```text
第12章【承】夜访旧档
[任务] 一句话，来自章名骨架，硬约束
[场景1] 地点/在场人物 → 发生什么 → 落点（80~120字）
[场景2] …
[场景3] …
[边界] 本章允许揭示：…；本章严禁触及：…（属第X章的…）
[钩子] 章末悬念（30~50字）
[字数] 2000（场景1:700 / 场景2:700 / 场景3:600）
```

用 `[标签]` 纯文本而不是 JSON：按行切片零改造兼容，作者也能在 textarea 里直接手改。场景数与字数分配和写章端同源（`sceneCountRange` / `sceneWordBudget`），杜绝「详纲切 3 场景、写章要 5 场景」的错配——那种错配会把详纲的字数分配与逐场景扩写预算一起打乱。

详纲生成时**输入全程裁剪**，从源头堵剧透：只给本章任务 + 后 2 章任务标题（仅作禁止越界的红线，不得写入本章）、本卷已登场的世界盘、本卷相关的主线切片；不给全书骨架、不给全书梗概，`bible.truths` 一个字都不进。生成结果过 `outlineLeakScan`：真相连续重合 ≥15 字直接拒绝入库，其余越界项以黄条提示、由作者拍板。

### 上下文精简开关

写作区的「细纲驱动」开关（状态存 `project.outlineDriven`，随书落库，默认关闭）决定单章写作的上下文口径：

| | 关闭（完整上下文） | 开启（细纲驱动） |
|---|---|---|
| 剧情依据 | 精简细纲本章切片 + 全书梗概 + 滚动摘要 + 故事线 + 编年史 + 前文召回 | **只有本章详纲**（唯一剧情依据） |
| 前文尾部 | 2000 字 | 800 字 |
| 世界块 | 全量（规则块永不省略） | 只取规则块，上限 1500 字 |
| 卷战略 | 全量 | 只留本卷弧线故事 + 卷末钩子 + 本卷禁回收清单 |
| 地图分层 | 注入 | 不注入（含后续层名称，本身是剧透源） |
| 伏笔 | 全量未回收 | 只留保护期内与本章可回收的，≤5 条 |
| 文风 / 禁用词 / 写作规则 / 范例 | 全保留 | **全保留**（管「怎么写」，不引起剧情漂移） |
| 场景清单 | 请 AI 规划 | 详纲已切好场景时确定性解析，**省一次请求** |

实测数据（接近真实成书体量的样本：梗概约 4800 字、编年史 / 前文召回 / 滚动摘要各上千字；`npm test` 里 `prompts.test.js` 固化了这组断言）：

| | system | user | 合计 | 详纲占整份 | 详纲占 user 段 |
|---|---|---|---|---|---|
| 关闭 | 2,973 字 | 15,824 字 | 18,797 字 | 2.69% | 3.20% |
| 开启 | 3,196 字 | 1,673 字 | 4,869 字 | **10.39%** | **30.25%** |

user 段砍掉约 89%，详纲在 user 侧的权重提升约 9.5 倍。真实成书的 system 段约 7,800 字（含完整文风档案与反模板规则），代入后详纲占整份约 6.9%；改造前的实测基线是 **0.4%~0.8%**。绝对百分比会随书的体量浮动（百万字书的梗概与编年史远大于本样本），有意义的是这个相对幅度。

开启后 system 侧还会换两条规则：规则 3 从「如有细纲，按细纲完成本章事件」变成「**本章详纲是唯一剧情依据，详纲未写的事件一律不得出现**」；新增规则 13 作为正向出口——「篇幅必须靠详纲给定场景的过程、对话与感官细节写足，**严禁靠新增剧情事件填字数**；某场景字数不足时只能加深该场景已有的动作、对话与感官层，不得引入新事件、新人物、新设定」。

关闭开关时上下文与改造前逐字节一致（有回归测试守着），两种模式可以放心 A/B 对比。
...
**细纲写完了怎么办？**
细纲剩余不足 3 章时页面会提示续写，AI 基于本卷相关的主线切片紧接规划下一批章节，追加进细纲，百万字不断地图。续写结果同样过越界监管：红线拒绝入库，其余以提示条列出、由你决定是否手改。

**旧书没有详纲、也没开细纲驱动会怎样？**
完全兼容。`outlineDetail` 缺失时自动降级用精简细纲地图的本章切片；`outlineDriven` 默认关闭，关闭时的上下文与改造前逐字节一致（有回归测试守着）。旧书的续写、审核、导出不受任何影响，可随时开关做 A/B 对比。
