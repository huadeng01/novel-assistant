...
import { allGenres, getWorldview, worldviewText, worldviewTropeBlock, isOverridden } from '../lib/worldviews/index.js'
...
export default function InspirationModal({ genre, apiKey, onPick, onClose }) {
  ...
  const gen = async (g) => {
    ...
    try {
      ...
      // 世界模板只取 brief 档（世界架构+力量体系+意象词库），维持「低权重参考」定位不膨胀；
      // 套路对照块单独作为第 4 参传入，把「反套路」从模型自由发挥变成照单规避（旧两字段数据时为空串，自动降级）
      const wv = getWorldview(g)
      const { data, engine } = await inspirationJSON({ apiKey, messages: inspirationMessages(g, worldviewText(wv, 'brief'), seed, worldviewTropeBlock(wv)) })
