...
// 续写页专用文风分析：与书库的 styleAnalyzeMessages（sampleNovel 抽样 7 段）本质不同——
// 这里针对「粘贴 / 导入的待续写原文」做整体分析，不做抽样丢弃：粘贴的可能是节选而非全文，字字都要看。
// 复用 STYLE_SYSTEM 的六维度与标点护栏，输出同构 JSON（style_profile / habits / samples），下游按同一套「皮骨」逻辑处理。
export function continueStyleMessages({ text }) {
  return [
    { role: 'system', content: STYLE_SYSTEM },
    { role: 'user', content: `以下是【待续写的原文】（可能是全书节选，请就所给的全部文字整体提炼写作习惯，不要只概括其中局部片段）：\n\n${text}` },
  ]
}
...
6. 若用户初始提问为爽文并含「核心驱动与升级链」（前/中/后期目标），factions 的登场节奏（unlock_volume）与 conflicts 的阶段（start/end_volume）应参考承接该升级链，使前/中/后期目标对应到不同层级的势力与冲突（参考非强制）。
【输出协议】严格按以下 JSON 格式输出，不要输出任何其他内容：
...
4. 章号从第 1 章严格连续编号；
5. 若本书为爽文（圣经或本卷故事含升级链/爽点），本卷细纲应参考其升级链：爽点桥段与升级目标对应本卷阶段，避免与前文已用的同层级套路重复，逐级升级（参考非强制）；
6. 直接输出细纲正文，不要输出任何解释性文字。`,
    },
    {
      role: 'user',
...
// 爽文选题（向导页「灵感」按钮）：为所选题材生成 5 个爽文开局选题，每个选题按"写清三件事"的初始提问格式输出。
// 用户选定爽文选题后由开书流程生成该书完整世界观；题材世界模板仅作背景参考（力量体系/意象可借用，不定死世界）。
// 爽文机制与「文艺向反套路」相反：套路即爽点——高频套路不是要规避的撞车，而是读者百看不厌的情绪按钮。
// 核心法则：主角全程开挂、打脸节奏极快、反派降智送经验、金手指简单粗暴、情绪宣泄优先、逻辑为爽感让路。
// 爽文骰：高温采样只增加用词噪声、不增加概念多样性——相同提示词每批都会收敛到最概率的那几条爽点。
// 所以每批随机抽 3~4 个正交的爽文要素轴注入提示词，把随机性从「措辞层」提升到「概念层」，跨批选题才真正拉开差异。
// 8 大爽点（power-fantasy satisfaction beats）：每批至少命中一个作为选题的核心情绪按钮。
export const POWER_FANTASY_BEATS = [
  { name: '装逼打脸', desc: '主角低调或被人看轻，随后当众亮出真实实力/身份，把嘲讽者狠狠打脸' },
  { name: '扮猪吃虎', desc: '主角伪装成废物/普通人，暗藏顶级实力，关键时刻一鸣惊人、反差拉满' },
  { name: '逆天改命', desc: '主角出身低微或被判死刑，靠金手指逆转命运、一路逆袭登顶' },
  { name: '获得奇遇', desc: '主角意外获得系统/神器/传承/随身老爷爷，从此开挂' },
  { name: '以弱胜强', desc: '主角以低境界/弱势身份越级斩杀强敌，反差感爆棚' },
  { name: '众人震惊', desc: '旁观者与大佬集体目瞪口呆，用他人的震撼反衬主角的逆天' },
  { name: '报仇雪恨', desc: '主角对曾经羞辱/背叛/伤害自己的人展开酣畅淋漓的清算' },
  { name: '突破升级', desc: '主角快速突破境界/等级，实力飙升，不断解锁新能力' },
  { name: '获得收集', desc: '寻宝/囤积/捡漏/集齐，收获他人梦寐以求的资源，满足收集癖' },
  { name: '躺赢锦鲤', desc: '不费吹灰之力就成功，咸鱼/锦鲤/天选，老天爷喂饭吃' },
  { name: '团宠被爱', desc: '被大佬/全员偏爱宠溺，万人迷，走到哪都有人保驾护航' },
  { name: '揭秘探奇', desc: '探索秘境/揭开真相/满足好奇，世界观层层揭秘的求知爽' },
]
// 爽文流派/题材套路（sub-genres）：决定选题的整体气质与开挂方式。系统流单列一族（子类最多），其余为经典与新兴流派。
export const POWER_FANTASY_SCHOOLS = [
  // 系统流一族（金手指即系统，按玩法细分）
  { name: '系统流·签到', desc: '每日签到白嫖顶级奖励，躺着变强，签到即巅峰' },
  { name: '系统流·红包群', desc: '进神仙/大佬红包群，抢红包得逆天能力与资源' },
  { name: '系统流·聊天群', desc: '万界聊天群，与各路大佬交易装逼，白嫖神通功法' },
  { name: '系统流·万界', desc: '绑定万界系统按任务穿梭诸界，每个世界都是打脸舞台' },
  { name: '系统流·直播', desc: '在异界/古代开直播，观众打赏加成，隔空装逼圈粉' },
  { name: '系统流·模拟器', desc: '人生/诸天模拟器，先推演后行动，永远选最优解' },
  { name: '系统流·抽奖', desc: '绑定抽奖面板，欧皇体质，一发入魂抽出神装' },
  { name: '系统流·反派', desc: '绑定反派系统专治各种主角，收割气运与机缘' },
  { name: '系统流·气运', desc: '操控/掠夺气运，好事自动上门，敌人倒霉透顶' },
  { name: '系统流·复制', desc: '复制他人天赋/功法/血脉，偷学一切，越抄越强' },
  // 经典流派
  { name: '无敌流', desc: '开局即无敌/满级，看点是降维打击与扮猪吃虎' },
  { name: '重生流', desc: '重生回到过去，凭先知先觉抢机缘、报前仇、囤资产' },
  { name: '都市修仙', desc: '现代都市里的修仙者，隐世高手在红尘中装逼打脸' },
  { name: '长生躺赢流', desc: '长生不老/无限寿命，苟着躺赢，耗死一代代敌人' },
  // 新兴爽点流派
  { name: '全民转职流', desc: '世界游戏化，觉醒唯一/隐藏职业，一路碾压同级' },
  { name: '词条流', desc: '人生可编辑词条，给万物加词条，白嫖顶级属性' },
  { name: '御兽流', desc: '契约神兽灵宠，养成进化，以兽海碾压强敌' },
  { name: '诸天流', desc: '自主穿越诸天万界，掠夺功法/血脉/机缘壮大自身' },
  { name: '幕后黑手流', desc: '苟在幕后布局操控全局，深藏不露，坐收渔利' },
  { name: '召唤流', desc: '召唤华夏英灵/远古强者为己作战，以众凌寡' },
  { name: '灵气复苏流', desc: '灵气复苏觉醒异能，抢占先机，成为时代弄潮儿' },
  { name: '洪荒流', desc: '穿越洪荒拜师圣人，抢占机缘，一路证道成圣' },
  { name: '神医医道流', desc: '一手神医妙术悬壶济世，扮猪吃虎打脸权贵' },
  { name: '鉴宝捡漏流', desc: '火眼金睛捡漏鉴宝，一夜暴富，打脸看衰者' },
  { name: '快穿虐渣流', desc: '女频：穿梭小世界完成委托，虐渣打脸、攻略大佬' },
  { name: '团宠马甲流', desc: '女频：多重隐藏马甲被全员团宠，层层掉马打脸' },
]
// 金手指（简单粗暴，一句话说清规则，不纠结科学与合理性）。
export const GOLDEN_FINGERS = [
  '签到系统', '神豪系统', '万界聊天群', '神仙红包群', '重生先知', '无敌修为（开局满级）',
  '透视/读心异能', '时间静止/减速', '随身老爷爷（顶级传承）', '复制/掠夺他人能力',
  '抽奖面板', '气运之子（好事自动上门）', '万物升级系统（可强化一切）', '杀敌爆装备/爆修为',
]
// 开局打脸对象：反派降智送经验的靶子，负责傲慢轻敌、层层加码、主动送脸。
export const FACE_SLAP_TARGETS = [
  '退婚的未婚妻及其新欢', '瞧不起主角的岳父/家族长辈', '开除或羞辱主角的上司老板',
  '抢走功绩的同事/同门', '当众嘲讽的天才/纨绔', '背叛过主角的旧人', '势力庞大的地头蛇/豪门',
]
// 主爽点驱动（8 种）：爽文不等于打脸，每本书可由一种主驱动牵引，各带一种「升级链」（前→中→后期目标阶梯）。
// 打脸只是其中一种驱动；成长/获得/揭秘/掌控/团宠/躺赢/复仇各有自己的升级链。元素库参考项，非强制。
export const POWER_FANTASY_DRIVES = [
  { name: '打脸', ladder: '反派梯队', desc: '前期小反派→中期势力→后期大Boss，层层打脸' },
  { name: '成长', ladder: '境界阶梯', desc: '前期瓶颈→中期大境界→后期登顶' },
  { name: '获得', ladder: '收集链', desc: '前期小宝物→中期神器→后期至宝集齐' },
  { name: '揭秘', ladder: '真相链', desc: '前期小谜团→中期区域真相→后期终极真相' },
  { name: '掌控', ladder: '掌控链', desc: '前期小局→中期势力网→后期天下棋局' },
  { name: '团宠', ladder: '情感链', desc: '前期个别宠爱→中期全员团宠→后期极致偏爱' },
  { name: '躺赢', ladder: '纪元链', desc: '前期小躺赢→中期熬死对手→后期见证纪元' },
  { name: '复仇', ladder: '仇人链', desc: '前期小仇→中期大仇→后期终极仇' },
]
// 立意倾向（番茄榜「标签即立意」）：一组标签+一句话钩子+主驱动+升级链+爽点密度/节奏，作为软倾向参考（可采纳可突破）。
// 灵感弹窗：常规小说=不注入爽文库；自由爽文=无倾向；立意倾向=软倾向。流派/金手指不由用户选，纯 AI 自主参考。
export const POWER_FANTASY_STANCES = [
  { id: 'system', name: '系统开挂流', tags: ['系统前置', '签到/面板', '爽点密集'], hook: '金手指第一句亮出，靠系统/签到碾压', drive: '打脸', ladder: '反派梯队', density: '极密：每章至少一个爽点', pace: '快节奏，开篇即高潮' },
  { id: 'mortal', name: '凡人流·无系统', tags: ['凡人流', '无系统', '慢热', '智斗积累'], hook: '无金手指，靠机缘+算计+毅力慢慢爬', drive: '成长', ladder: '境界阶梯', density: '中：阶段性爆发', pace: '厚积薄发，每卷一次大爆发' },
  { id: 'reborn', name: '重生囤积·复仇流', tags: ['重生', '囤物资', '无限空间', '黑化复仇'], hook: '重生带先知/空间，囤积资源+黑化复仇', drive: '获得', ladder: '收集链', density: '密', pace: '复仇节点集中爆发' },
  { id: 'misread', name: '迪化反差流', tags: ['迪化', '搞笑', '苟', '别人脑补我强'], hook: '主角只想苟/其实很弱，却被全员脑补成大佬', drive: '掌控', ladder: '掌控链', density: '中', pace: '反差揭晓时爆发' },
  { id: 'underdog', name: '身份逆袭流', tags: ['低身份', '炮灰觉醒', '穿书', '登顶'], hook: '开局炮灰级低身份，觉醒后一路逆袭登顶', drive: '打脸', ladder: '反派梯队', density: '密', pace: '身份揭晓即爆发' },
  { id: 'longlife', name: '长生躺赢流', tags: ['长生', '无系统', '慢节奏', '见证纪元'], hook: '长生不死，躺看纪元更迭，熬死一代代敌人', drive: '躺赢', ladder: '纪元链', density: '低', pace: '纪元级淡爽' },
]
// 每批随机抽 3~4 个轴、每轴随机取一个选项，拼成「本批参考种子」（软参考，保批间差异，非硬约束）
const POWER_FANTASY_DICE = [
  { axis: '主打爽点', options: POWER_FANTASY_BEATS.map((b) => b.name) },
  { axis: '流派', options: POWER_FANTASY_SCHOOLS.map((s) => s.name) },
  { axis: '金手指', options: GOLDEN_FINGERS },
  { axis: '开局对手/目标', options: FACE_SLAP_TARGETS },
]
// 无参纯随机：从 4 轴随机抽 3~4 个作「本批参考种子」，供 AI 优先参考（可微调），保证连点生成批间有差异。
export function inspirationSeed() {
  const pickOne = (a) => `${a.axis}=${a.options[Math.floor(Math.random() * a.options.length)]}`
  const axes = [...POWER_FANTASY_DICE].sort(() => Math.random() - 0.5).slice(0, 3 + Math.floor(Math.random() * 2))
  return axes.map(pickOne).join('；')
}

// tropes：该题材的「高频套路清单」原文（getWorldview(genre).tropes），可为空串。
// 爽文机制下高频套路不再是要规避的撞车，而是百看不厌的爽点素材库——有内容时作为「可照用/强化/叠加」的爽点参考注入，
// 为空（旧数据 / 自定义题材未填）时该块整体跳过，不影响其余爽文法则。
// opts: { mode, stance, seed }。mode='normal'=常规小说（不注入爽文库）；'free'=自由爽文（注入元素库，无倾向）；'guided'=倾向爽文（注入元素库+stance软倾向）。
// 元素库（驱动/爽点/流派/金手指/升级链）全部为参考素材，AI 自主参考组合，非硬约束；流派/金手指不由用户选。
export function inspirationMessages(genre, worldview, tropes, { mode = 'free', stance = null, seed = '' } = {}) {
  // 常规小说：不叠加爽文机制，人物动机合理、冲突循序渐进、反派有自身立场
  if (mode === 'normal') {
    return [
      {
        role: 'system',
...
固定人设：…；固定背景：…。
核心冲突与开局钩子：…。
请帮我设计前期节奏：如何铺陈人物与冲突、如何埋下第一个悬念。`,
      },
      { role: 'user', content: `题材：${genre}。请给出 5 个人物动机合理、冲突循序渐进、彼此差异明显的${genre}小说选题（常规叙事，不强制爽文套路）。` },
    ]
  }
  const seedBlock = seed
    ? `\n【本批参考种子（随机，优先参考，可微调）】\n${seed}\n5 个选题优先参考这些要素展开，并分别侧重不同组合，避免全部挤在同一套路；如与更好创意冲突可微调。\n`
    : ''
  const tropeBlock = tropes ? `\n【该题材高频爽点套路（参考素材，可照用/强化/叠加）】\n${tropes}\n` : ''
  const stanceBlock = stance
    ? `\n【用户倾向立意（软参考，可采纳可突破）】\n立意：${stance.name}\n标签：${stance.tags.join(' / ')}\n钩子：${stance.hook}\n主驱动：${stance.drive}（${stance.ladder}）\n爽点密度：${stance.density}\n节奏：${stance.pace}\n说明：以上为用户偏好倾向，5 个选题中 1~2 个贴近即可，其余自由发挥，不必全部套用。\n`
    : ''
  const drives = POWER_FANTASY_DRIVES.map((d) => `${d.name}（${d.ladder}：${d.desc}）`).join('\n')
  const beats = POWER_FANTASY_BEATS.map((b) => `${b.name}（${b.desc}）`).join('\n')
  const schools = POWER_FANTASY_SCHOOLS.map((s) => `${s.name}（${s.desc}）`).join('\n')
  const fingers = GOLDEN_FINGERS.join('、')
  return [
    {
      role: 'system',
...
${drives}
· 爽点（${POWER_FANTASY_BEATS.length} 种，每个选题主打 1~2 个）：
${beats}
· 流派（可作主基调或混搭）：
${schools}
· 金手指（可选用，规则一句话说清）：
${fingers}
【爽文基调（参考，非强制）】
1. 开局快给爽点：第一章就露锋芒，不做漫长铺垫；
2. 爽点密集：压抑→反转→释放的循环要紧凑，开篇尽快迎第一波高潮；
3. 情绪宣泄优先：先爽后合理，逻辑为爽感让路；
4. 升级链参考：为每个选题按其主驱动给出前/中/后期三段目标（打脸=反派梯队；成长=境界阶梯；获得=收集链；…），后期目标明显强于前期，保证「前期爽完后期还有更强的可追」；非打脸驱动不必写打脸对象；境界/实力数值须对照【力量体系】阶梯——开局落在低段留足升级空间，升级链前<中<后严格递增，不得开局即巅峰或境界倒挂（如开局修为高于前期目标）；
5. 5 个选题各自命中不同的驱动/爽点/流派组合，世界观、主角身份、开局场景互不重复；
6. 每个选题写清三件事：① 主角人设（姓名+表身份/真实身份+1~2 核心关系）；② 金手指与背景（一句话开挂规则+一句话世界观，可自由突破）；③ 开局爽点钩子（第一波爽点的具体场景）。
【输出协议】严格按以下 JSON 格式输出，不要输出任何其他内容：
{"ideas":[{"title":"选题名（12 字内）","brief":"完整初始提问文本"}]}
brief 必须按此模板写成一段话：
我想写一部「流派名」${genre}爽文，主打「爽点名」。主角…，…，拥有「金手指」。
固定人设：…；固定背景：…。
核心爽点与开局钩子：…。
核心驱动与升级链：主驱动「X」；前期…；中期…；后期…。
请帮我设计前期爽点节奏：如何开局就给爽点、如何安排升级链与钩子。`,
    },
    { role: 'user', content: `题材：${genre}。${stance ? `用户倾向立意：${stance.name}（${stance.tags.join('/')}，软参考）。` : '自由发挥，无倾向。'}世界模板与元素库仅供参考可自由突破${seed ? `；本批参考种子：${seed}` : ''}。请给出 5 个爽点密集、彼此差异明显的${genre}爽文选题，每个带主驱动与前/中/后期升级链。` },
  ]
}

// 灵感扩充/迭代：用户点击某选题后，注入该题材 full 世界模板（含完整境界阶梯），把窄 brief 扩充成完整自洽的开书初始提问；
// userNote 非空时为「按用户想法迭代修改」。核心是境界硬校准——对照力量体系阶梯修正倒挂、强制升级链递增。
export function inspirationExpandMessages({ brief, genre, worldview, stance = null, userNote = '' }) {
  const stanceBlock = stance ? `\n【本书立意倾向（软参考）】${stance.name}：${stance.hook}；主驱动 ${stance.drive}（${stance.ladder}）。\n` : ''
  const noteBlock = userNote ? `\n【用户的修改想法（必须落实）】\n${userNote}\n请在当前灵感基础上按此想法修改，其余保持自洽。\n` : ''
  const task = userNote ? '按用户想法修改并扩充' : '扩充'
  return [
    {
      role: 'system',
...
${brief}
【扩充要求】
1. 境界硬校准（重点）：严格对照【力量体系】的境界阶梯——主角开局修为/能力必须落在阶梯的低段（留足升级空间，不得开局即巅峰）；升级链的前/中/后期境界必须逐级严格递增，后期明显高于前期；原选题若出现境界倒挂或数值矛盾（如开局修为高于前期目标、跳级、用错境界名），一律以阶梯为准修正；
2. 丰富而非注水：补全主角弧光、核心冲突、金手指规则、升级链各阶段（对手/目标/爽点）、开局钩子场景与前期节奏，但保持一段话的紧凑初始提问体例；
3. 忠于原选题的核心创意（主角/金手指/主驱动/开篇钩子），只修正矛盾、补充细节，不推倒重写；
4. 沿用该题材世界模板的力量体系与意象，不得自创与模板冲突的境界名或设定。
【输出协议】严格按以下 JSON 格式输出，不要输出任何其他内容：
{"brief":"扩充后的完整初始提问文本（一段话）"}
brief 体例：我想写一部「流派名」${genre}爽文，主打「爽点名」。主角…。固定人设：…；固定背景：…。核心爽点与开局钩子：…。核心驱动与升级链：主驱动「X」；前期…；中期…；后期…。请帮我设计前期爽点节奏：…。`,
    },
    { role: 'user', content: userNote ? `请按我的想法修改并扩充这个${genre}灵感，注意境界对照力量体系阶梯递增、不得倒挂。我的想法：${userNote}` : `请扩充这个${genre}灵感选题，重点校准境界阶梯（开局低段、升级链严格递增、修正倒挂），输出完整自洽的初始提问。` },
  ]
}
